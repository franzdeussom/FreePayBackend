...
exports.sendMail = async (email, code, username) => {
  ...
  const filePath = path.resolve(__dirname, "../controllers/files/data.json");
...
exports.calculateGain = (
  ...
  if (derniere_connexion != this.getCurrentFormatedDate()) {
    if (checkDateResult.isStillValid) {
      const jourSansConnexion = this.getRestantDayOfPack(
        derniere_connexion,
        0
      ).jourRestants; //nombre de jour que l'utilisateur a fait sans se connecter

      if (jourSansConnexion > 0) {
        //au
        const dailyGain = ((prix * Taux_Rendement) / 100) * jourSansConnexion;
        return {
          newSole: Number.parseFloat(soldeUser) + dailyGain,
          expireIn: checkDateResult.jourRestants,
          isStillValid: checkDateResult.isStillValid,
        };
      }
  ...
  return {
    newSole: Number.parseFloat(soldeUser),
    expireIn: checkDateResult.jourRestants,
    isStillValid: checkDateResult.isStillValid,
  };
};
...
exports.getRestantDayOfPack = (dateSouscription, duree) => {
  const [jourSouscription, moisSouscription, anneeSouscription] = String(
    this.getCurrentFormatedDate()
  )
    .split(".")
    .map(Number);
  const [jour, mois, annee] = this.getPackExpireDate(dateSouscription, duree)
    .split(".")
    .map(Number);

  const SouscriptionDate = new Date(
    anneeSouscription,
    moisSouscription - 1,
    jourSouscription
  );
  ...
  return {
    jourRestants: duree > 0 ? nbrJour : Math.abs(nbrJour),
    isStillValid: nbrJour >= 0,
  };
};
